...
class GameIndexer extends AbstractIndexer
{
    ...
    {
        $this->preBuildTeams(); // with id ??
        $this->preBuildCorrections();
        $this->preBuildStatus();
...
SQL;
    ...
    private function preBuildTeams(): void
    {
    ...
    {
        //$sql = sprintf('SELECT * FROM correction %s', $id ? sprintf(' WHERE id = ');
        /*foreach($this->handler->iterator($sql) as $row) {
            $this->corrections[$row['standing_id']][] = $row;
        }*/
    }

    private function preBuildStatus(): void
    {
        $sql = 'SELECT ss.id, ss.standing_id, ss.status_id, s.code, s.name FROM standing_status ss LEFT JOIN status s ON s.id = ss.status_id';
        foreach($this->handler->iterator($sql) as $row) {
